                        ...
                        for(my $q=0;$q<=$#dropout;$q++){
                            ...
                            for(my $r=0;$r<=$#wd;$r++){
                                ...
                                # the name of the job file has the parameter configuration
                                    ...
                                    printf JOB "--epochs 35 --batch_size 32 --split 1 --use_pretrain 1 --eb_n_layers 8 --molecule_pooling_method 'con' --self_hidden_dim %d --self_n_layers %d --patience %d --factor %f --init_lr %f --gnorm %f --dropout %f --weight_decay %f --out_dir %s > %s 2>&1 ", $hidden_size, $Nlayer, $lrdecay, $decaystep, $initlr, $gnorm, $dropout, $wd, $modelpath, $outfile;  # your matlab script 
                                    print JOB "\n";
